...
class Tastingmonail extends CI_Controller {

		public function __construct()
        {
                parent::__construct();
				$this->load->model('tastingmonail_model','model');
        }

	public function index(){		
	$array=array();
	$array['modeltitle']='Tastingmonail';
	$array['data']=	$this->model->getData(); 	
	$this->load->view('admin/Tastingmonail',$array);
	...
	extract($_POST);	
	$this->db->where('id', $id);
	$res=$this->db->delete('tbl_tastingmonail');
	if($res==1){
		$response['status']=200;
