...
class Cmsprocess extends CI_Controller {

		public function __construct()
        {
                parent::__construct();
				$this->load->model('cmsprocess_model','model');
        }

	public function index(){		
	$array=array();
	$array['modeltitle']='Cms Process';
	$array['data']=	$this->model->getData(); 	
	$this->load->view('admin/Cmsprocess',$array);
	...
	extract($_POST);	
	$this->db->where('id', $id);
	$res=$this->db->delete('cms_process');
	if($res==1){
		$response['status']=200;
	...
	public function save(){
		...
		$this->form_validation->set_rules('cms_id', 'cms_id','required');
